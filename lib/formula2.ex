...
defmodule Formula2 do
  ...
  def parse_and_validate(formula, var_names, options \\ @default_validate_opts) do
    formula
    |> parse()
    |> validate(
  ...
  def parse_and_eval(formula, formula_env, options \\ []) do
    formula
    |> parse()
    |> eval(Map.merge(options[:env] || default_env(), formula_env))
  end
  ...
  def validate(ast, %{} = env, var_names, options \\ @default_validate_opts)
